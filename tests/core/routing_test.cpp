...
TEST(TrackOutput, NothingIsTheMaster) {
  ...
  ASSERT_EQ(p.tracks[1].output, bus);

  p = set_track_output(std::move(p), "a1", "");
  EXPECT_TRUE(p.tracks[1].output.empty());
  EXPECT_EQ(routed_output(p, p.tracks[1]), nullptr) << "empty means the master";
}

TEST(TrackOutput, OnlyASubmixCanBeFed) {
  Project p = set_track_output(sequence(), "a1", "a2");
  EXPECT_TRUE(p.tracks[1].output.empty()) << "an ordinary lane has clips, not an input";
}

TEST(TrackOutput, ATrackCannotFeedItself) {
  Project p = sequence();
  p.tracks[1].submix = true;
...
TEST(TrackOutput, ARouteThatWouldCloseALoopIsRefused) {
  ...
  const std::string one = p.tracks[3].id;
  const std::string two = p.tracks[4].id;

  p = set_track_output(std::move(p), one, two);
  ASSERT_EQ(p.tracks[3].output, two);

  p = set_track_output(std::move(p), two, one);
  EXPECT_TRUE(p.tracks[4].output.empty()) << "two already feeds one";
}

TEST(TrackOutput, ASubmixThatHasGoneIsReadAsTheMaster) {
  Project p = add_submix_track(sequence(), "Dialogue");
  p = set_track_output(std::move(p), "a1", last_id(p));
  p.tracks.pop_back();  // the bus is deleted underneath the route

  EXPECT_FALSE(p.tracks[1].output.empty()) << "the route is kept, so undo can bring it back";
  EXPECT_EQ(routed_output(p, p.tracks[1]), nullptr) << "but it is heard on the master meanwhile";
}

TEST(TrackOutput, ALoopArrivingInAFileIsReadAsTheMaster) {
  // Not buildable through `set_track_output`, and a file can say anything.
  Project p = add_submix_track(add_submix_track(sequence(), "One"), "Two");
  p.tracks[3].output = p.tracks[4].id;
  p.tracks[4].output = p.tracks[3].id;

  EXPECT_EQ(routed_output(p, p.tracks[3]), nullptr);
  EXPECT_EQ(routed_output(p, p.tracks[4]), nullptr);
}
...
TEST(Sends, OneIsAddedAndThenChangedRatherThanDoubled) {
  ...
  ASSERT_EQ(p.tracks[1].sends.size(), 1u);
  EXPECT_DOUBLE_EQ(p.tracks[1].sends[0].level, 0.25);
  EXPECT_FALSE(p.tracks[1].sends[0].pre_fader) << "post-fader is Premiere's default";

  p = set_send(std::move(p), "a1", bus, 0.5, true);
  ASSERT_EQ(p.tracks[1].sends.size(), 1u) << "the same destination is one send, not two";
  EXPECT_DOUBLE_EQ(p.tracks[1].sends[0].level, 0.5);
  EXPECT_TRUE(p.tracks[1].sends[0].pre_fader);

  p = remove_send(std::move(p), "a1", bus);
  EXPECT_TRUE(p.tracks[1].sends.empty());
}
...
TEST(Sends, ASendCountsAsARouteWhenLoopsAreChecked) {
  ...
  ASSERT_EQ(p.tracks[3].sends.size(), 1u);
...
TEST(BusRoutes, ABusFeedingABusIsOrderedTheWholeWayDown) {
  ...
  const std::string stems = p.tracks[3].id;
  p = add_submix_track(std::move(p), "Dialogue");
  const std::string dialogue = p.tracks[4].id;
...
TEST(BusRoutes, ASendToNowhereIsDropped) {
  Project p = sequence();
  p.tracks[1].sends.push_back(Send{.to = "gone", .level = 0.5});
  const std::vector<BusRoute> routes = bus_routes(p);
  ASSERT_EQ(routes.size(), 2u);
...
TEST(BusAudibility, SoloingABusSolosWhatFeedsIt) {
  ...
  const std::string bus = last_id(p);
  p = set_track_output(std::move(p), "a1", bus);
  p.tracks.back().solo = true;

  EXPECT_TRUE(is_track_audible(p, p.tracks[1])) << "a1 feeds the soloed bus";
  EXPECT_FALSE(is_track_audible(p, p.tracks[2])) << "a2 goes straight to the master";
  EXPECT_TRUE(is_track_audible(p, p.tracks.back()));
}
...
TEST(RoutingFile, RoutingSurvivesARoundTrip) {
  ...
  EXPECT_TRUE(read.tracks.back().submix);
  EXPECT_EQ(read.tracks.back().label, "Reverb");
  EXPECT_EQ(read.tracks[2].output, bus);
  ASSERT_EQ(read.tracks[1].sends.size(), 1u);
  EXPECT_EQ(read.tracks[1].sends[0].to, bus);
  EXPECT_DOUBLE_EQ(read.tracks[1].sends[0].level, 0.35);
  EXPECT_TRUE(read.tracks[1].sends[0].pre_fader);
}
...
TEST(RoutingFile, ASendNamingNothingIsNotReadBack) {
  Project p = sequence();
  p.tracks[1].sends.push_back(Send{.to = "", .level = 0.5});
  const auto loaded = from_json(to_json(p));
  ASSERT_TRUE(loaded.has_value()) << loaded.error();
  EXPECT_TRUE(loaded->project.tracks[1].sends.empty());
}
