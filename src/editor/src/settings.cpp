...
std::string to_json(const Settings& settings, int indent) {
  ...
  if (!settings.media_cache_folder.empty()) {
    out["media_cache_folder"] = settings.media_cache_folder;
  }
...
std::expected<Settings, std::string> settings_from_json(std::string_view text) {
  ...
  settings.media_cache_folder =
      document.value("media_cache_folder", settings.media_cache_folder);
