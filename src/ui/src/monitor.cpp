...
void MonitorView::set_frame(const ImageView& frame) { frame_ = frame; }

void MonitorView::clear_frame() { frame_ = ImageView{}; }
...
Rect MonitorView::picture() const {
  ...
  const double aspect = frame_.empty() ? canvas_aspect_ : frame_.aspect();
