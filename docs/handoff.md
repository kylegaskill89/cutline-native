...
**If you are picking this up to carry on, read Â§9 first** â€” it says exactly
...
native decisions, `docs/spec.md` for the product â€” it is the authority on exact
numeric behaviour, carried over from the TypeScript version â€” `docs/releasing.md`
...
the whole reason the rewrite exists, and it is done â€” see `README.md` for the
...
| Old app | `github.com/kylegaskill89/cutline` â€” dead, kept as reference |
| Old app, local | `d:\Videos\VideoTrimmer` â€” holds `design.md` (the rewrite spec) and `summary.md` |
...
binding between them â€” 2392 of the 2772 tests.
...
| `ui` | **everything** â€” FFmpeg, D3D12, Skia | very long, but it is the one the app needs |
| `ci` | what the GitHub workflow runs | â€” |
...
- `--check [dir]` â€” headless: builds the whole interface in every theme, lays
...
- `--benchmark` â€” layout and paint timings per theme and per window size.
...
- A new *editing operation* â†’ `core`, pure, tested against the spec's numbers.
- A new *effect* â†’ `render/src/effects.cpp` **and** `render/effect_catalog.cpp`.
...
- A new *control* â†’ `ui/controls.hpp`. Themeable, tested with `RecordingPainter`.
- *What a panel shows* â†’ `editor/*_binding.hpp`, as plain data.
- *Wiring* â†’ `tools/cutline/main.cpp`. It is large; that is fine, it is the
  composition root.
- *Something the interface needs but must not wait for* â†’ `app`, as a cache with
...
- *Something derived from footage that is worth keeping between sessions* â†’
  `app/media_cache.hpp`, which both of those caches consult before they decode.
  Read Â§5.8 of the gaps document before adding a third kind: the key and the
  unit are the two decisions that go wrong.
- *A preference* â†’ `editor/settings.hpp` and a row on a page in `main.cpp`. Â§9
  has the recipe and the rule for deciding whether it is a preference at all.
- *A per-sequence setting* â†’ `core::Project`, serialised, with a control in
...
**British spelling in prose and identifiers** where it is a real word â€”
`colour_row`, "behaviour", "initialise". Not in API names that mirror an
external convention (`Color`, `set_color`) â€” the widget layer uses `Color`
...
corner radii, shadows *and metrics* â€” bevelled chrome needs different spacing
...
backend that outputs a list of draw calls â€” "does an XP button draw its bevel
...
before every commit.** Its project is deliberately awkward â€” two levels of bin,
a labelled entry, 29.97 so the drop-frame control exists at all â€” because a page
...
- **Focus did not leave a field on a click elsewhere**, so a multiline title â€”
  which writes to the document only when focus goes â€” could never be committed.
...
and `settings.json` now carries the theme, which is the dangerous one â€” every
...
that changes without input â€” a progress line, a count coming down, a playhead
moving on its own â€” and for popups, which are drawn over the window.
...
been clicked and had not come on â€” the button moves when its label changes
...
and nothing matches â€” it needs `CharSet.Unicode`. An `EnumWindows` callback
...
any project that is all audio â€” which is what every mixer test was â€” so a mixer
...
accepts a whole multiple selection as quoted names separated by spaces â€”
`"a.mp4" "b.mp4" â€¦` â€” so navigating to the folder and pasting forty of them
...
`[System.Windows.Forms.Screen]::PrimaryScreen.Bounds` reports 3840Ã—2160 you are
in physical pixels and when it reports 3072Ã—1728 you are in logical ones, and a
...
trusting a click, and check `WindowFromPoint` is the app before pressing â€” the
...
- **A long source playing with sound, for longer than a window lasts.** Audio
  ...
  by tests â€” the samples either side of a refill, and that the block size cannot
  change them â€” but *hearing* seventy-five seconds go by and the next stretch
...
- **A source placed as it is marked** â€” that insert, overwrite and a drag from
...
- **The application running on WARP, for more than a few minutes.** Choosing the
  ...
  black too. It did not reproduce â€” the same actions afterwards were handled
...
capture on it. The routing itself was right first time â€” Interview set to
...
`fill_audio_panel` now and rebuilt on a deferred `mixer_stale` flag â€” deferred
...
clip's own gain of 0.6 falling to 0.076 â€” which is 0.6 at -18 dB, the amount the
box was showing.
...
- **It is slow, and it gets slower.** The first six tiles took about a minute,
  ...
  total â€” nearer twenty-five seconds a source by the end. Nothing blocks: the
  ...
  helps the second time. That it *decelerates* is the part worth chasing â€” a
...
media the project no longer has. It predates the batch import â€” a single-file
import undone does the same â€” and it is a refresh that does not check that what
it is showing still exists.

#### Measured: what reading a source actually costs

Asked whether a media cache would smooth playback. It would not â€” playback is
video-decode bound and reads none of this â€” but measuring the question found
...
waveforms went from minutes to **1.4 s**. The full write-up is Â§5.8 of the gaps
document.
...
"over eleven minutes" to nine â€” and timing the pieces separately showed audio
...
the script did not expect, the theme changed under it â€” every theme has its own
metrics, so every coordinate moved â€” and the owner was using the desktop. The
...
Closing Â§5 added a preroll and a postroll around a looped range, and driving
...
the sequence â€” where the player stops itself, clears `running`, and is therefore
...
`Player::play` sends a finished player back to zero â€” so `seek` then `play`,
...
range marked 4â€“6 s in an eight-second sequence:
...
the picture** â€” finding the reddest column in a one-pixel row of the timeline
...
recording when it changes, which is the only way to see evenness â€” an average
...
**Forward is even** â€” nothing above 38 ms â€” but runs at about 40 fps on 4K60
...
3329, 3850, 4389, 4898 ms and so on: a gap of **509â€“540 ms, over and over**,
...
60 fps content played backwards at 1x is half a second â€” which is the number
...
**The fix is to stop doing the work in one lump where it can be seen** â€” not to
...
twice a second are down to one in ten. Forward is untouched â€” 315 distinct
frames against 319, nothing above 50 ms â€” because the prefetch only runs when
...
last turn took will, after a quarter-second hitch, jump fifteen frames â€” half a
run â€” so the next stretch has half as many turns to read ahead in and stalls
...
182, 4577, 8922, 11177, 13334 â€” evenly spaced. Nothing is out of sync as a
...
Before those, the last of it â€” the cursors, the snap line, and the two
clip-menu framing rows â€” was driven and is recorded below.
...
names it â€” which is how "the trim zone is eight pixels wide and reads `size-we`
...
All eight phases are complete, `docs/spec.md` Â§21 and Â§18 are done, and there is
...
| Â§1 Effects | done |
| Â§2 Timeline | done |
| Â§3 Monitors | done bar a deferred row (see the audit list at the foot of that file) |
| Â§4 Project panel | done bar "new sequence from a clip", which needs more than one sequence |
| Â§5 Application settings | **done** â€” every row built or declined with a reason, see that file's Â§5.6 |
...
audio clock. Export with hardware encode, chosen at runtime â€” NVENC, QSV, AMF,
then x264/x265 â€” with progress and cancel. Project save/load. Four themes,
...
**Since 0.3.0** â€” the 24 unreleased commits, which are most of what this handoff
is about:

- **Proxies, whole.** `media::write_proxy` transcodes; `app::ProxyBuilder` runs
  them one at a time on a worker; `Project â–¸ Make Proxies / Use Proxies / Stop`
...
- **Relinking** â€” `Project â–¸ Relink Media`, repointing one pool entry repairs
  every clip that used it.
- **Bins** â€” nested folders in the project, saved with it, dragged into,
  renamed, deleted. `core/pool.hpp` is the model, `MediaBrowser` draws the tree.
- **The project panel, rebuilt** â€” sortable column headings, an icon view whose
  tiles scrub as the pointer crosses them, labels on sources that clips inherit
  at placement, and a right-click menu.
- **`settings.json`** â€” the preferences file, and everything that now persists.
- **The settings windows** â€” Preferences and Project Settings as real modal
...
- **Â§5 closed** â€” undo depth, recovery copies kept, preroll and postroll, and a
...
- **The media cache**, and the reading it exists to avoid. Waveforms and
  ...
  emptiable from Preferences â–¸ Media Cache. Under it, two fixes to how a source
...
not in any section's table because they are not Premiere comparisons â€” they are
...
- **Every preview frame drains the GPU twice.** `Compositor::compose` ends with
  `submit()` then `wait_for_idle()`, and `display_texture` â€” which runs
  immediately after it on every preview frame â€” does the same again. Each of
  ...
  **The wait cannot simply be deleted** â€” `upload_planes` maps and rewrites one
...
- **Forward and reverse can disagree about which frame a moment is.** The
  ...
  (see Â§9), while `Source::covering` â€” the run of remembered frames, which is
  what reverse playback reads from â€” still takes the last frame *at or before*
...
- **Retimed audio is still decoded whole.** A clip with a speed or a reverse on
  it is stretched once, up front, across its whole span â€” a window cannot be
  handed to something that reads its input end to end. Every other clip is
  windowed now (see Â§8's note on `AudioMixer`), so this is the one case left
...
- **Filmstrips fill a large pool slowly, and slow down as they go.** Ten
  ...
  blocking, so it degrades well â€” but browsing a folder of rushes should not be
  a thing you wait out, and the deceleration says something is accumulating.
  Both of these are written up under *Driven: forty in the pool* in Â§5.
...
- ~~**Forward playback runs at about 40 fps on 4K60**~~ â€” **fixed.** It was
  ...
  three was answered with the frame it had just shown. 4K60 now plays at 56â€“57 of
  60 on screen. Written up under *Measured: what playback actually costs* in Â§9,
  along with the four plausible performance fixes that measured out to nothing
  before the real cause was found.
- ~~**Changing the preview quality stalls for over a second.**~~ â€” **fixed.**
...
- **Nothing in the test suite ever resizes a real window.** The pixel tests draw
  ...
  `WM_SIZE` â†’ recreate-the-swapchain path had never been exercised until somebody
  dragged the window under a driver â€” which is how the crash in *Traps* was
...
- **Proxies** â€” `media/transcode.hpp` writes one, `app/proxies.hpp` runs them.
  ...
  by repeating and dropping rather than emitted one for one â€” without that a
...
- **Bins** â€” `core/pool.hpp`. A bin holds nothing: it is a name and a place in a
  ...
  containers disagreeing, and it is why deleting a folder cannot lose footage â€”
...
- **The settings file** â€” `editor/settings.hpp`. The fourth thing under
...
- **The settings windows** â€” `open_settings_dialog` in `tools/cutline/main.cpp`.
  ...
  one box. They are modal, which on Windows means disabling the owner â€” and the
  owner is re-enabled *before* the dialog is destroyed, so the system has an
  enabled window to give the foreground to.

- **Drop-frame timecode** â€” `core/time.hpp`. Read the comment on
...
- **The monitor's transform handles** â€” `ui/monitor.hpp` for the gesture and
  ...
  not â€” get them the wrong way round and the picture is only wrong when the
...
- **The master fader and the meter** â€” `audio/meter.hpp` measures and
  ...
  limiter, so "over" means the limiter is having to work â€” a meter after it can
...
- **In and out points** â€” marked from the buttons or from I and O, drawn along
  the foot of the ruler, saved with the project, offered to export as "only the
  marked range".
- **Transitions** â€” `editor/transitions.hpp`. The interesting part is not
  ...
  elsewhere** â€” the model will happily store things the renderer ignores, and
  the binding layer is where that gets caught.
- **Audio effects** â€” the bottom of `editor/effects_binding.hpp`. Worth reading
...
- **Markers** and **interpolation chips** â€” both small. The marker keys reuse
...
- **Colour mattes and adjustment layers** â€” `editor/generators.hpp`, alongside
...
- **Waveforms** â€” `app::WaveformCache` and the drawing in `ui/timeline.cpp`. The
  ...
  next mouse move.
- **Scopes** â€” `render/scopes.hpp` counts and `ui/scopes_view.hpp` draws, and
  ...
  two numbers that are not obvious â€” the vectorscope's reach, which is 140 and
...
- **Multi-select** â€” `TimelineView::selection` and the `Marquee` mode. Worth
  ...
  reaches for a sweep.
- **Thumbnails** â€” `app::ThumbnailCache` and the tiling in `ui/timeline.cpp`.
  ...
  short clip of a long file may repeat a frame. That is a deliberate limit â€”
...
- **The volume rubber band** â€” `GainBand` in `ui/timeline.hpp`. The whole of it
  ...
  needs one** â€” this is the second time an "unreachable" item turned out to be
...
three others back by hand, so the labels were wiped on every save â€” found by
...
gutter out of that same width. Fixed in the widget â€” it reserves the gutter
whether or not a bar is showing â€” but the shape of the bug is worth remembering:
...
false at exactly the instant there is something left to decide â€” loop round, or
...
on screen ever said so â€” which reads as a dead control and is a dead repaint.
...
some theme. Elide it, give it a row of its own, or both â€” and make `--check`
...
*written* â€” the frame rate, drop-frame â€” has to call `show_playhead` itself.
...
`Session::set_playhead` snaps to the frame grid, so the J/K/L shuttle â€” which
added a couple of milliseconds per turn of the loop â€” read the playhead back,
...
sequence â€” all storable, all silently skipped. Whenever you expose something new,
...
catches this now â€” it walks with the nearest clipping ancestor and counts
anything sticking out of it horizontally â€” but only because a third button in the
...
the swapchain crashed the application â€” an access violation in `skia.dll`, seven
fresh instances out of seven â€” because `SkiaWindow::resize` dropped the wrapped
...
was three times in five rather than every time â€” and once tracing was added it
...
case.** `AudioMixer` decoded everything up front for exactly this reason â€”
WASAPI wants a buffer every few milliseconds and a decode is tens of them â€” and
...
a ten-minute file marked the whole file covered â€” and every clip of it on the
...
second `WithFootage` in an anonymous namespace still collides â€” the suite is keyed
...
worth at the tail â€” and `gain_value_at` walks in time order, so it answered with
...
caught up when the model was rebuilt on release â€” so the drag showed one thing
...
keyframe list â€” the drawing, the core's evaluator â€” assumes it is in order, and
...
**`docs/premiere-gaps.md` Â§6, audio, is closed** â€” and with it sections 1 to 6.
...
**Pick the next section** â€” see *The next thing* below.

The rest of this section is about **Â§5, application settings**, which closed
before it. It is kept because it is the part of the application whose rules are
easiest to get wrong from the outside: every row in Â§5 is either built or
declined with a written reason, that file's Â§5.6 is the reasons, and Â§5.7 is
...
|---|---|---|---|
| **Preferences** | the person | `%APPDATA%\Cutline\settings.json` | Settings â–¸ Preferencesâ€¦ |
| **Project settings** | the cut | inside the `.cutline` file | Project â–¸ Project Settingsâ€¦ |
...
Preferences has ten pages â€” General, Appearance, Audio Hardware, Playback,
Graphics, Labels, Timeline, Proxies, Media Cache, Auto Save â€” and everything on
...
`premiere-gaps.md` Â§5.6: memory reserved, appearance brightness, and scratch
disks. The media cache row was declined and then built once somebody measured
what its absence cost â€” Â§5.8 is that story. Keyboard customisation was pulled
out into a section of its own, being larger than the rest of Â§5 together.

### Three things from Â§5 worth knowing before you touch them
...
- **Recovery copies are timestamped and pruned**, `<name>-<digest>-<stamp>`,
  ...
  names sort chronologically as text â€” ordering needs neither the filesystem's
...
- **The renderer choice is read once, when the device is made**, and the page
  ...
  one chosen â€” the device still falls back to WARP on its own.
...
autosave interval, the label palette, the proxy size, the output device â€” which
...
2. **Read and write it** in `settings_from_json` / `to_json`, clamping on the way
   in. A hand-edited file is not hostile, but it is not careful either â€” every
   number in there is clamped and the enums are read by name.
3. **A row on a page** in `tools/cutline/main.cpp` â€” `build_*_page`. Use
...
4. **Call `save_settings(app)`** from the control's `on_commit`, not
   `on_change` â€” a dragged number would otherwise write the file once a pixel.
...
6. **Run `--check`.** It lays out every page of both windows in all four themes.
   ...
   real one â€” see the trap about lengths.
...
substantial one, and it was pulled out of Â§5 for being larger than the rest of
...
and counting how many grabs differ â€” playback is clocked by the sound card, so
...
`fps.ps1`, in the session scratchpad alongside the driver described in Â§5; the
fixture is one 4K60 clip on V1 with its audio on A1.

| what | displayed |
|---|---|
| Debug build, 4K60 source | **13.5 fps**, 75 ms between pictures |
| Release build, 4K60 source | **38.3 fps**, 25 ms â€” since fixed, see below |
| Release, same thing at 1/2 preview quality | 38.7 fps â€” *no change* |
...
at.** The interface paint alone is 14â€“54 ms per frame under Debug against
0.8â€“3.1 ms under Release â€” `cutline --benchmark` prints both. Anything about
...
cleanly â€” the program monitor, and the timecode field beside the timeline, which
...
is **1/1000** â€” so a 60 fps screen capture is stamped 0, 17, 33, 50, 67, 83 ms
...
`kFrameEpsilon` was 1e-4 â€” a tenth of a millisecond. So the frame stamped 33 did
...
for â€” **exactly the 40 distinct frames a second** that was measured, and exactly
...
completely â€” with the tolerance put back to 1e-4 the test reports 15 distinct
...
half of the stack â€” and why the reference clip, `Boiler.mp4`, cannot be the only
...
**Preview quality is for effects, and it was measured on the wrong thing.** Full
37.1 fps, 1/2 32.5, 1/4 33.1 â€” all on bare footage, where the canvas the
compositor renders into is not what the time is going on, so of course it made
no difference. That is not what the setting is for. Its job is the case where a
stack of effects is being drawn over the whole canvas, which is per-pixel GPU
work and *does* scale with the canvas: half the resolution is a quarter of the
shading. **It has never been measured with effects on, and that is the
measurement that would say whether it works.**
...
response. Only the compositor is sized to the canvas â€” a decoder's surface pool
comes from its media's own dimensions â€” so it now resizes the compositor and
keeps the sources.
...
exactly as a Gaussian blur's sigma is, and both were left at full size â€” so at
...
*history* â€” scrub about first to build up several seeks, then check the tally is
...
so nobody spends the afternoon on it again â€” all four were reverted, and all four
...
write over a picture not yet shown â€” nothing sequences the two queues. Rotating
...
the screen" from "the picture inside it is stale" â€” sample the monitor and the
...
One caveat on all of the above: it was measured on a 3840Ã—2160 *virtual* display
adapter, not on the machine's real 3440Ã—1440 120 Hz panel. The Debug-versus-
...
- 3016 tests pass under the `ui` preset, 2591 under `release`. Set
...
- **The suite is stable under `-j`, and this has now been fixed twice.** The
  ...
  still in six more fixtures â€” `encoder_test`, `transcode_test` and
  ...
  **Every fixture that writes to the temp directory now has `_getpid()` in the
  name.** If you add one, do the same â€” and note that a counter, an address and a
  test name all look unique and are not.
- `--check` reports 2747 widgets, 0 empty, 0 outside, 0 clipped, 0 squeezed, in
  all four themes â€” run it with something *in* the media cache as well as with
  it empty, since the Delete button only exists when there is something to
  delete.
- **Where the work is:** sections 1 to 6 of the gaps document are closed. Â§6.2
...
- **Three things about the audio path are worth knowing before you touch it.**
  A submix is a flag on an audio track, not a third `TrackKind` â€” it has a
  ...
  every clip by `plan_audio` â€” which is where Premiere has it and the only
...
- **Not driven yet:** the Fit Clip dialogue (it needs a project carrying four
  ...
  strip's fx button, and â€” for Â§6.2's last rows â€” a lane routed into a submix
...
- Released as **`v0.4.0`**. The tag is what the workflow builds the installer
  ...
  start â€” which is the first thing that can fail, deliberately. **Do not tag** without being asked â€”
...
- **They write the direction; you write the code.** They review lightly and
  ...
  decisions â€” make the call, state it plainly, and say why.
...
- **`docs/buglist.md` is their channel, not yours.** They put faults in it in
  ...
  of your own â€” that is what `premiere-gaps.md` is for.
